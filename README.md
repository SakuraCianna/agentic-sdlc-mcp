...
`security-focused` treats a passing, app-backed mature secret-scanner CI check (Gitleaks, TruffleHog, Secretlint, detect-secrets, or an explicit GitHub Secret Scanning check) as primary evidence only after its Actions run, PR head SHA, base-ref workflow, and known scanner action pinned to a full commit SHA are verified. Same-name jobs/statuses, checks from unknown Apps, and mutable action tags are not trusted as clean-scan proof. If evidence is incomplete, scanner policy changes in the PR, no recognized scanner ran, or the scan is pending/failed, the review fails closed. The built-in added-line assignment heuristic is supplemental and is never reported as proof that a repository is secret-free.

The supplemental scanner also reports `DynamicSecretConstruction` when a credential-like field is assembled through string concatenation, template interpolation, `.join()`, common decoding calls, multiline expressions, or identifiable computed/nested credential properties. Explicit environment-variable and `secrets.*` sources, comments, removed lines, and credential metadata such as `tokenCount` are excluded to reduce noise. This is patch-local risk detection, not whole-program data-flow analysis: indirect construction across functions/files or unknown computed keys still requires trusted scanners, CodeQL/SAST, and human review.
