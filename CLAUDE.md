...
- Secret review has two distinct evidence tiers. `src/security/secret-scanner-evidence.ts` recognizes mature CI scanners (Gitleaks, TruffleHog, Secretlint, detect-secrets, or an explicit GitHub Secret Scanning check), but passing is trusted only after concrete workflow-run provenance verifies the PR head and a base-ref workflow pinned to a known scanner action's full commit SHA. Same-name commit statuses/jobs, mutable action tags, incomplete CI sources, and PR changes to workflows/Gitleaks policy fail closed. `src/review/pull-request-review.ts`'s `scanPatchForSecrets` remains a conservative patch-local heuristic: it detects literal assignments and `DynamicSecretConstruction` patterns but is not cross-file data-flow analysis and must never be described as a complete secret scan.
