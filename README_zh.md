...
`security-focused` 将由可信 GitHub App 提供的成熟密钥扫描器 CI check（Gitleaks、TruffleHog、Secretlint、detect-secrets 或明确的 GitHub Secret Scanning check）作为主要扫描证据。同名 commit status 或未知 App 的 check 不能证明扫描通过。证据不完整、PR 修改扫描策略、未运行、仍在 pending 或扫描失败时都会采用 fail-closed 结论。内置的新增赋值行启发式检查只作为补充，绝不会被表述为“仓库不存在密钥泄露”的证明。
