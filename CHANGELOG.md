...
- Added the first T9 critical-evaluation slice: six scripted two-tool security/release/evidence/handoff chains replay through the real MCP 2.0.0 client, consume the preceding structured result, pin canonical argument digests, score 100 with no critical violations, and fail closed for reversed order, skipped gates, forbidden tools, live writes, degraded Issue evidence, external fetch, sockets, and live Issue creation. The separate prompt-injection source/channel-parity slice remains in progress.
...
- Raised the global coverage regression floors to 94% statements, 89% branches, 94% functions, and 95% lines; after adding the full tool matrix, Inspector/Conformance runner boundaries, bounded secret-scanner regressions, the T7/T8 evaluation suites, self-hosted reviewer feedback regressions, and the first T9 critical-evaluation slice, the current 1375-test baseline is 96.54% statements, 91.29% branches, 97.17% functions, and 97.27% lines.
