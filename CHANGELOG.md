...
- Raised the global coverage regression floors to 94% statements, 89% branches, 94% functions, and 95% lines; after adding the full tool matrix, Inspector/Conformance runner boundaries, bounded secret-scanner regressions, the T7/T8 evaluation suites, self-hosted reviewer feedback regressions, and the complete T9 critical/channel-parity evaluation, the current 1424-test baseline is 96.61% statements, 91.51% branches, 97.39% functions, and 97.34% lines.
