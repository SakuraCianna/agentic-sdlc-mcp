...
- Prevented the supplemental PR secret heuristic from treating response `tokenEstimate`/`tokenBudget` metadata as credentials; real API/access tokens and client secrets remain fail-high. This regression was found by running the Agentic SDLC MCP reviewer against its own T10 PR.
