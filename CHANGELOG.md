...
- Raised the global coverage regression floors to 94% statements, 89% branches, 94% functions, and 95% lines; after adding the full tool matrix, Inspector/Conformance runner boundaries, and bounded JSON secret-scanner regressions, the current 1304-test baseline is 96.37% statements, 91.22% branches, 97.09% functions, and 97.14% lines.
