...
- Added the complete T9 critical-evaluation suite: six scripted two-tool security/release/evidence/handoff chains replay through the real MCP 2.0.0 client and consume the preceding structured result; a versioned 14-source prompt-injection matrix covers Issue/PR fields, comments, repository files/rules, workflow/job/log text, and GitHub errors. Nine consumed boundaries run through real MCP/policy/error paths, while five sources not collected by public tools are locked to zero ingestion calls. Actual evidence-packet Markdown/structured summaries reach the same gate conclusion, omitted source records fail closed, semantic/HTML-comment variants are detected, and source digests are derived from each raw input. Fixed traces score 100; reversed order, prompt-injection handoff, skipped/incomplete gates, forbidden tools, live writes, target expansion, external fetch, sockets, and live Issue creation fail closed.
- Added T10 versioned response budgets for all 13 public tools, measured through the real MCP 2.0.0 client and fixed GitHub fixture. Reports hard-gate GitHub calls, recursive items, Markdown characters, structured JSON bytes, timeout/cancellation, and source-specific violations; the deadline signal is verified through the MCP handler, and only a complete all-passing 13-report run publishes the ignored artifact. Fixed nearest-rank P95 samples retain headroom, while the documented UTF-8-bytes/4 token estimate remains observational only.
- Added the T11 versioned GitHub fault matrix and `npm run eval:faults`: 11 cases cover 401/403/404/422/429/500, GraphQL partial errors, request timeout/cancellation, 301-item truncation, required-field loss, and duplicate API responses through the real MCP 2.0.0 client. Each case binds an affected public tool to a different aggregate tool, preserves available sources, blocks clean promotion, forbids external network/live writes, and publishes an ignored artifact only after a unique all-passing 11/11 run.
- Prevented the supplemental PR secret heuristic from treating response `tokenEstimate`/`tokenBudget` metadata as credentials; real API/access tokens and client secrets remain fail-high. This regression was found by running the Agentic SDLC MCP reviewer against its own T10 PR. The two already-committed occurrences of one synthetic AWS-format fixture are exempted from Gitleaks only through separate rule, immutable commit, path, and line-pattern matches; current tests construct the value from fragments so later commits need no exception.
