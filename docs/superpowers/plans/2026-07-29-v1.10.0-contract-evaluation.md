...
> **T1 结果（2026-07-29）：** `v1.9.0` baseline 固定到 `3e1cdbb2d591ba482903f53579f1f76cc95ff1c4`，通过真实 MCP client 收集 13 tools/5 resources；普通检查只读，生成器缺少 `--update` 会拒绝写入。比较器覆盖输入有效 schema/default、输出 union/required/constraint、boolean schema、JSON enum、`integer ⊂ number`、annotations 与复杂关键字人工复核，并使用跨平台稳定排序与 prototype-safe key 处理。Node 22/24 做快速当前契约比较，独立 Node 24 job 通过隔离 checkout 逐字重放 baseline；历史 discovery 在有硬 deadline、checkout 外 cwd、凭据环境清理的短生命周期子进程内执行，Windows 连续 3 次实测均在句柄释放后保持 1 个 worktree、0 个临时目录。收尾测试扩展到 1062 个用例，覆盖 timeout/cancellation、证据降级和分页非法边界，全局门槛提升到 94/89/94/95。
