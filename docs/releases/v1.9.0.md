...
> 状态：开发中，尚未发布。最终发布日期、Git commit、npm 包和 GitHub Release 以发布流程产物为准。
...
- 新增公共 prompt-injection 检测层，覆盖：
  ...
  - 零宽字符和双向控制符混淆。
- 高置信内容不会进入 agent-facing Markdown 或 handoff prompt；原始值保留在 structuredContent 中，供调用方按“不可信数据”审查。所有成功工具响应同时提供 `_meta` 与 `structuredContent.trustBoundary`，避免 structured-only 客户端丢失信任边界。
...
- Prompt-injection 检测是可解释的高置信模式防线，不是自然语言安全证明；未命中的恶意文本仍必须视为不可信数据。
...
- `@modelcontextprotocol/sdk` 当前传递依赖 `@hono/node-server` 1.x，npm audit 会报告其 Windows `serve-static` 中危公告。本项目不调用 Hono `serve-static`；审计给出的 force 修复会把 MCP SDK 降到不满足当前 HTTP 契约的版本，因此本版本不采用。发布前仍要求 production/full audit 的 high 门禁为零，并跟踪上游 SDK 修复。
...
2. 不修改现有 GitHub token 或仓库权限。
3. 保存失败的 tool name、输入 schema 版本、Node 版本和脱敏错误。
4. 创建 Issue，并说明是 Markdown、structuredContent、Node 运行时还是 GitHub 权限回归。
