...
> **T5 进展（2026-08-01）：** Inspector 2.0.0 已由隔离 lockfile 精确固定，并通过显式 ad-hoc stdio target 从进程外启动正式 `dist/index.js`。本地 Windows Node 24 已通过 53 files/1188 tests 和 Inspector 黑盒，验证 legacy `2025-11-25` initialize、server version、13 tools、5 resources 的逐项 read、显式 `create_issue_set dryRun:true` 且 created 为 0，以及 invalid schema/unknown resource URI 的机器退出与错误码；所有 `tools/call` 都只指向 dry-run 工具。runner 使用官方 `-e` 把固定环境传给目标，真实 server 未写临时 harness marker 时会失败；它不改写 HOME、不读取默认 Inspector catalog，独立 storage/OAuth/client state，屏蔽产品全局配置文件，清除真实 GitHub 凭据，并在纯 stdio 检查中禁用全部 fetch/TCP；Linux Node 24 证据等待本切片 PR CI。该结果不是 MCP 官方认证，也不证明 modern `server/discover`。
