...
#### 3. 增强 `plan_from_context` 的任务类型识别
...
#### 2. 改善 `create_issue_set` 预览体验

当前 dry-run 返回 `previewTitles`，安全性很好，但对人类审查还不够完整。下一步应让 dry-run 预览像一个提交前确认页面。
...
| 优先级 | 方向 | 原因 |
|---|---|---|
| P0 | 通用 Agent 冒烟测试文档 | 让任何 MCP 客户端都能快速验证 server 可用 |
| P0 | `repo_context` 增强 | 所有后续计划和审查都依赖上下文质量 |
| P1 | `plan_from_context` 支持 `workType` | 解决模板化计划不贴合任务的问题 |
| P1 | `issueDrafts` 与 `create_issue_set` 打通 | 形成 Plan -> Issue 的闭环 |
| P2 | 合并门禁增强 | 从 CI 查询升级为工程治理判断 |
| P2 | `.agentic-sdlc.yml` | 支持不同仓库的策略差异 |
| P3 | `sdlc_evidence_packet` | 形成产品级差异化和审计闭环 |

## 成功指标

短期成功指标：

- 任意 MCP 客户端中的 AI coding agent 能在 5 分钟内完成 MCP 配置验证
- `repo_context` 能返回足够 agent 开工的上下文，不需要用户手动补充 README/package/scripts
- docs-only、feature、bugfix、security、release、infra 任务能生成明显不同的计划
- dry-run issue 预览能让人类判断是否允许真实创建

中期成功指标：

- `plan_from_context` 输出可直接 dry-run 创建 issue
- PR 门禁结果能解释为什么通过、失败、等待、缺审查或存在策略缺口
- security/release 类任务能自动生成更严格的审查项
